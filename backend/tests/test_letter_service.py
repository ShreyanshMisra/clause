import pytest
from app.letter_service import (build_letter_html, build_report_html, render_letter_pdf,
                                _blocking_url_fetcher)
...
def test_report_html_renders_findings_and_escapes():
    html = build_report_html([
        {"category": "Deposit", "severity": "illegal", "statute_citation": "M.G.L. c.186 § 15B",
         "explanation": "forfeiture clause", "legal_reasoning": "violates 15B",
         "recommended_action": "demand return"},
    ], damages_rows=[("Deposit", "M.G.L. c.186 § 15B", 4500.0)])
    assert "Deposit" in html and "forfeiture clause" in html
    assert "demand return" in html and "$4,500" in html

def test_report_html_escapes_model_values():
    html = build_report_html([{"category": "<script>", "severity": "x", "explanation": "e"}])
    assert "<script>" not in html and "&lt;script&gt;" in html

def test_report_html_empty_findings():
    html = build_report_html([])
    assert "No issues were flagged" in html
